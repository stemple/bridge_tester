...
void serialEvent(Serial myPort) {
  //bridgeData = myPort.readString();
  //if (bridgeData != null) {
  //println("Got bridge data");
  //redraw();
  //}
}
...
void draw()
{
  ...
  String newBridgeData = myPort.readStringUntil('\n');
  println("Old Bridge Data", bridgeData);
  if(newBridgeData != null) {
    bridgeData = newBridgeData;
    println("New Bridge Data:", newBridgeData);
  }
  println("Current Bridge Data:", bridgeData);
 
  // for debugging purposes
  //bridgeData = debugLoad + ", " + debugLoad + ", " + debugLoad + ", " + debugLoad + "\n";
  if (started == true && pause == false) {
    //Check if we have a reading. If so, record it.
    if (bridgeData != null) {
      background(255);
      bridgeData = trim(bridgeData); //gets rid of any whitespace or Unicode nonbreakable space
      //println(bridgeData); //Optional, useful for debugging. If you see this, you know data is being sent. Delete if  you like.
      float loadVals[] = float(split(bridgeData, ',')); //parses the packet from Arduino and places the valeus into the sensorVals array. I am assuming floats. Change the data type to match the datatype coming from Arduino.
